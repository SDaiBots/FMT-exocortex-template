...
Проверка обновлений — **лёгкая** (расширение `day-open.before.updates.md`, один `gh api` ~0.5с) → строка «Наличие обновлений: нет / есть». **НЕ запускать `update.sh --check`** (тяжёлая сетевая посверка, таймаут; полное обновление — отдельной командой `/iwe-update`). + проверка Base-репо (FPF, SPF, ZP) на отставание от origin. Обновления → «Требует внимания». Scout report не проревьюен → «Требует внимания».
