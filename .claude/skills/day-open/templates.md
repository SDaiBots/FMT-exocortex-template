...
> **Стиль:** `##`-заголовки секций (Obsidian-совместимо; хук-валидатор `protocol-artifact-validate.sh` требует ≥3 `##`). Без `<details>`. Приоритет = светофор (🔴🟡🟢). Результат = краткое название из Strategy.md.
> **Обязательные секции** (проверяет валидатор): `План на сегодня` · `Календарь` · `IWE за ночь` · `Разбор заметок` · `Итоги вчера`. Обязательно: мультипликатор `~N.Nx`, строка `Mandatory check`, `Carry-over`, бюджет `~Xh РП / ~Yh физ`.
...
**Mandatory check:** [WP-7 + контентный РП / «не сконфигурирован»]. **Carry-over** из Day Close [дата]: [список].
...
| Наличие обновлений | 🟢/🟡 | нет / есть (краткие изменения) |
| Scout | 🟢/🟡/🔴 | [N находок] |

## Наработки Scout (разбор)

> Отчёт за DD мес — N находок, M capture-кандидатов. **Статус ревью:** ⬜ не проверен / ✅ проверен
> Опциональна: если Scout не сконфигурирован — секцию можно опустить.

## Разбор заметок

> Источник: Note-Review (вчера) или мини-триаж (Day Open шаг 1c).
> **Если все обработаны** → «Все заметки обработаны (коммит HASH, N заметок). Carry-over: нет.»
...
> Топ-3 черновика из `drafts/` для работы сегодня. Опциональна (если `content_editor.enabled: false` — опустить).
